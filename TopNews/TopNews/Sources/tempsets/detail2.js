...
function setDetail(result, netType, isOn) {
        ...
        if (imgContextMap) {
            ...
            if(list){
                for(var j = 0,size = list.length; j < size ; j++) {
                    //                    if (window.jsbridge.imageExists(list[j])) {
                    ...
                    if (jsCallImgList(list[j])) {
                        ...
                        switch(netType) {
                            ...
                            default:
                                if (isOn) {
                                    var img = getImg("DEV_PLACEHOLDERIMG");
    ...
    var comments_line = document.querySelector("#comments_line");
    comments_line.style.display="block";
    
    var comments = document.querySelector("#comments");
    comments.style.display="block";
    
    jsCallDetailLoadOver();
...
function setRecommend(result) {
    ...
    var recommended_line = document.querySelector("#recommended_line");
    recommended_line.style.display="block";
